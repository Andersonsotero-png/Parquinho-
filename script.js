/* Main app JS - cleaned and modularized */
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("enviarWhatsApp").onclick = async () => {
    ...
    const file = document.getElementById("imagemMarketing").files[0];
    for (const id of ids) {
      const c = clients.find(x => x.id === id); if (!c) continue;
      const text = `${subject}\n\n${msg}`;
      if (navigator.canShare && file && navigator.canShare({ files: [file] })) {
        try { await navigator.share({ title: subject, text, files: [file] }); }
        catch (err) { window.open(`https://wa.me/${c.telefone}?text=${encodeURIComponent(text)}`, "_blank"); }
      } else {
  ...
  document.getElementById("enviarSMS").onclick = () => {
    const ids = getSelectedClients(); if (!ids) return;
    const msg = document.getElementById("mensagemMarketing").value || "";
    ids.forEach(id => { const c = clients.find(x => x.id === id); if (!c) return; window.open(`sms:${c.telefone}?body=${encodeURIComponent(msg)}`, "_blank"); });
  };
  ...
  document.getElementById("enviarEmail").onclick = async () => {
    ...
    for (const id of ids) {
      const c = clients.find(x => x.id === id); if (!c) continue;
      if (navigator.canShare && file && navigator.canShare({ files: [file] })) {
        try { await navigator.share({ title: subject, text: msg, files: [file] }); }
        catch (err) { window.open(`mailto:${c.email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(msg)}`, "_blank"); }
      } else {
        window.open(`mailto:${c.email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(msg)}`, "_blank"); }
      await new Promise(r => setTimeout(r, 180));
    }
